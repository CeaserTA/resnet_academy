...
return [
    ...
    'info' => [
         ...
         */
        'description' => "Resnet LMS backend API.\n\n"
            .'**Auth:** primary login is email + password (`POST /api/v1/login`), with Google OAuth '
            .'(`GET /api/v1/auth/google/redirect`) as a linked, non-primary method — both converge on '
            .'the same Sanctum-authenticated session (cookie-based, not bearer tokens; call '
            .'`GET /sanctum/csrf-cookie` first from a browser client). Endpoints marked "requires '
            .'auth" need an authenticated session; role-specific access (admin/instructor/student) is '
            .'enforced server-side via Policies and returns 403 otherwise.',
    ],

    'ui' => [
        'title' => 'Resnet LMS API',
    ],
